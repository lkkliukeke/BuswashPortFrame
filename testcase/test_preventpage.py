...
localexcel = public.get_xls('RecordsExcel.xlsx', 'prevent')
...
class TestPreventlPage:
    @pytest.mark.parametrize('excelist', localexcel)
    @allure.feature('test_preventpage')
    @allure.title('双重预防/预警模块')
    @allure.story('100%')
    def test_prevent(self, excelist):
        #设置url
